...
extern MATRIX divisione_tensore_ott(MATRIX S, int dimS, double d);
...
MATRIX trasposta_tensore(MATRIX S, params* input) {
    ...
    int n  = input->n;
    int nn = input->nn;
    MATRIX At = alloc_matrix(s, n*nn);

    for(int k = 0; k < s; k++)  //per ogni matrice nel tensore
        for(int i = 0; i < n; i++)
            for(int j = 0; j < nn; j++)
                At[(j*n + i + k*n*nn)] = S[(i*nn + j + k*n*nn)]; //T[j][i] = A[i][j]
    return At;
}

MATRIX divisione_tensore(MATRIX S, double d, params* input) {
    int s = input->s;
    int n = input->n;
...
void att(params* input){
    ...
    for (int i = 0; i < ns; i++) { //per ogni tensore nel dataset
