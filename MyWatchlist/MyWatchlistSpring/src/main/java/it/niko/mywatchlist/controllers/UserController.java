...
public class UserController {
    ...
    public ResponseEntity<?> getByUsername(@RequestParam(value = "username") String username) throws UserNotFoundException {
        User ret = userService.showUserByUsername(username);
        return ResponseEntity.ok(ret);
    }

    @GetMapping("/search/by_email")
    public ResponseEntity<?> getByEmail(@RequestParam(value = "email") String email) throws UserNotFoundException {
